...
// ... (QuickLinkCard en NewsCard komponente bly presies dieselfde) ...
const ArrowIcon = () => (
...
export default function HomePageClient({ latestNews }: { latestNews: NewsPostFeedItem[] }) {
    ...
    // 2. STATE OM TE BEPAAL OF ONS DIE VIDEO MOET WYS
    const [showVideo, setShowVideo] = useState(false);
    const [isDesktop, setIsDesktop] = useState(false);

    useEffect(() => {
        // Stel vas of ons op desktop is
        const isDesktopQuery = window.matchMedia("(min-width: 640px)");
        setIsDesktop(isDesktopQuery.matches);

        // Gaan na of video reeds gespeel is
        const hasVideoPlayed = sessionStorage.getItem('heroVideoPlayed');

        if (isDesktopQuery.matches && !hasVideoPlayed) {
            // Slegs as ons op desktop is EN video nog nie gespeel het nie,
            // stel die state om die video te wys.
            setShowVideo(true);
        }

        // Luister vir veranderinge in skermgrootte (opsioneel)
        const handler = (e: MediaQueryListEvent) => setIsDesktop(e.matches);
        isDesktopQuery.addEventListener('change', handler);
        return () => isDesktopQuery.removeEventListener('change', handler);
    }, []); // Loop slegs een keer op 'mount'

    // 3. FUNKSIE WAT LOOP AS VIDEO KLAAR IS
    const handleVideoEnd = () => {
        sessionStorage.setItem('heroVideoPlayed', 'true'); // Onthou vir die sessie
        setShowVideo(false); // Versteek die video-element
    };

    return (
        <div className="flex flex-col">
            {/* --- 1. HERO SEKSIE (Opgedateer met Video-logika) --- */}
            <section className="relative flex h-[65vh] min-h-[400px] w-full items-center justify-center bg-zinc-800 text-white overflow-hidden">

                {/* STATIESE VALSKEERM-PRENT (WYS ALTYD, lê agter) */}
                <Image
                    src={HERO_FALLBACK_IMAGE_URL}
                    alt="Hoërskool Brits Kampus"
                    layout="fill"
                    objectFit="cover"
                    className={showVideo ? "opacity-0" : "opacity-50"} // Bietjie donkerder as die video s'n
                    priority
                />
                <div className="absolute inset-0 bg-amber-600/10"></div>

                {/* VIDEO-ELEMENT (Wys slegs op Desktop, en slegs as 'showVideo' true is)
        */}
                {showVideo && isDesktop && (
                    <video
                        key="hero-video"
                ...
                >
                    ...
                    <Image
                        ...
                        width={96} // 24 in Tailwind (h-24 w-24)
                        height={96}
                        priority
                        className="mx-auto h-35 w-auto mb-5"
                    ...
                    <div className="mt-10">
                        <Link
                            href="/aansoek"
                            className="rounded-md border border-transparent bg-rose-900 px-8 py-3 text-base font-medium text-white shadow-lg transition hover:bg-rose-800 hover:scale-105"
                        >
                            Doen Aansoek
                        </Link>
                        <Link
                            href="/oor-ons"
                            className="ml-4 rounded-md border border-white bg-white/10 px-8 py-3 text-base font-medium text-white backdrop-blur-sm transition hover:bg-white/20"
                        >
                            Meer Oor Ons
                        </Link>
                    </div>
                ...
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="text-center">
                        <h2 className="text-3xl font-bold tracking-tight text-rose-900 sm:text-4xl">
                            Welkom by die aanlyn tuiste van die Britsies
                        </h2>
            ...
            {/* --- 3. VINNIGE SKAKELS SEKSIE (Bly dieselfde) --- */}
            ...
            {/* --- 4. NUUTSTE NUUS SEKSIE (Bly dieselfde) --- */}
