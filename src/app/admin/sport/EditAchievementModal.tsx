// src/app/admin/sport/EditAchievementModal.tsx
'use client';

import { useState } from 'react';
import { useFormStatus } from 'react-dom';
import Image from 'next/image';
import { updateSportAchievement } from './actions';
import type { DbSportAchievement } from '@/types/supabase';
import FloatingLabelInputField from '@/components/ui/FloatingLabelInputField';
import AchievementImageUploader from './AchievementImageUploader'; // <-- Gebruik die korrekte oplaaier

type EditModalProps = {
  achievement: DbSportAchievement;
  onClose: () => void;
};
...
export default function EditAchievementModal({ achievement, onClose }: EditModalProps) {
  ...
  const updateAction = async (payload: FormData) => {
    await updateSportAchievement(payload);
    onClose();
  };
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="text-lg font-semibold dark:text-white">Wysig Prestasie</h3>
        <p className="text-sm text-zinc-600 dark:text-zinc-400">{achievement.title}</p>
        ...
        <form action={updateAction} className="relative mt-4 space-y-4">
          ...
          <div className="flex items-center">
            ...
            />
            <label htmlFor="is_active_achieve_edit" className="ml-2 block text-sm dark:text-zinc-300">Aktief?</label>
          </div>
