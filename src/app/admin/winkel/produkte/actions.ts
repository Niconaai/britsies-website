...
// Hulpfunksie om vormdata na DbShopProduct-tipe te omskep
function getProductDataFromFormData(formData: FormData): Omit<DbShopProduct, 'id' | 'created_at'> {
    ...
    // --- REGSTELLING HIER ---
    // Kry die category_id as 'n string
    ...
    return {
        ...
        // --- REGSTELLING HIER ---
        // As die string leeg is (""), stuur 'null'. Andersins, stuur die uuid.
        ...
        options: null, // Ons hanteer JSONB later
  };
}
...
export async function createProduct(formData: FormData) {
  ...
  if (!productData.name || productData.price === null) {
    return redirect('/admin/winkel/produkte/create?error=Naam en Prys word vereis.');
  }
  ...
  if (error) {
    ...
    return redirect(`/admin/winkel/produkte/create?error=${encodeURIComponent(error.message)}`);
  }
